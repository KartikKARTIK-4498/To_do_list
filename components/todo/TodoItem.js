...
function TodoItem({ content, _id,description, important, task_done, setTodo,todoDescription, settodoDescription }) {
    ...
    return (
        <article className={classes.todoItem} data-id={_id} style={{
            display: "flex",
            alignItems: "center"
        }}>
            <div
            ...
            <h1 className={classes.todoContent} style={{ left:"-1rem"}}>{content}</h1>
            <hr/>
            <h3 className={classes.todoContent} style={{fontSize:"0.8rem", left:"-0.8rem"}}>{description}</h3>
            </div>
